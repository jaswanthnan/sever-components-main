  ...
  useEffect(() => {
    const fetchJob = async () => {
      try {
        ...
        setError(err.message || 'Could not fetch job posting');
      } finally {
        setFetching(false);
      }
    };
  ...
  if (fetching) {
    return (
      <div className="max-w-2xl mx-auto py-20 text-center space-y-3">
        ...
        <span className="text-sm font-bold text-slate-500 dark:text-slate-400">Loading posting details...</span>
      </div>
    );
  }
    ...
    <div className="max-w-2xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        ...
        <p className="text-slate-500 dark:text-slate-400 mt-2 text-sm font-medium">
          Modify the requirements or role parameters for this posting.
        </p>
      </div>
      ...
      >
        ...
        >
          {loading ? 'Saving Changes...' : 'Save Job Posting Changes'}
        </button>
      </form>
    </div>
  );
